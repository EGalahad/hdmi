...
class PPOPolicy(PPOBase):
    def __init__(
        ...
        self.reward_scales = torch.tensor(
            [
                self.cfg.object_reward_scale if name == "object_tracking" else 1.0
                for name in self.reward_groups
            ],
            device=self.device,
        )
        if (self.reward_scales <= 0).any():
            raise ValueError("Reward group scales must be positive")
    ...
    def _compute_advantage(
        ...
        def _global_mean_std(x, mask):
            if aa.is_distributed():
                ...
                local_sum = (x * mask.unsqueeze(-1)).sum(dim=(0, 1))
                local_sum_sq = (x * x * mask.unsqueeze(-1)).sum(dim=(0, 1))
                count = local_count.float().expand_as(local_sum).clamp_min(1)
