...
cs.store("hdmi_ppo_roa_train", node=PPOConfig(phase="train"), group="algo")
cs.store("hdmi_ppo_roa_adapt", node=PPOConfig(phase="adapt"), group="algo")
cs.store("hdmi_ppo_roa_finetune", node=PPOConfig(phase="finetune"), group="algo")
...
class PPOROA(PPOBase):
    def __init__(
        ...
        encoder_student_in_keys = [OBS_KEY, CMD_KEY, OBJECT_PCD_FEATURE_KEY]
        critic_in_keys = [
            OBS_PRIV_KEY,
            OBS_KEY,
            CMD_LONG_KEY,
        ...
        encoder_teacher_in_keys = [OBS_KEY, CMD_LONG_KEY, OBJECT_PCD_FEATURE_KEY]
        ...
        if self.cfg.phase == "adapt":
            self.object_pcd_encoder.requires_grad_(False)
    ...
    def train_op(self, tensordict: TensorDict):
        category_info = object_category_metrics(
            tensordict, self.cfg.object_category_names
        )
        ...
        with ScopedTimer("training.adapt.minibatch_loop", sync=False):
            for _ in range(2):
                for minibatch in make_batch(
                    ...
                    with torch.no_grad():
                        self._encode_object_pcd(minibatch)
        ...
        if not ("state_value" in keys and ("next", "state_value") in keys):
            with ScopedTimer(
                ...
                with tensordict.view(-1) as tensordict_flat:
                    self._encode_object_pcd(tensordict_flat)
                    self._encode_object_pcd(tensordict_flat["next"])
    ...
    def _update_ppo(self, tensordict: TensorDict):
        ...
        with ScopedTimer("training.policy.ppo.clip_grad", sync=PROFILE_SYNC_TIMERS):
            ...
            pcd_grad_norm = nn.utils.clip_grad_norm_(
                self.object_pcd_encoder.parameters(), self.cfg.max_grad_norm
            )
            if self.cfg.phase == "train":
        ...
        info["opt/grad_norm.object_pcd"] = pcd_grad_norm.detach()
